...
import {
  getHasntWorkoutClubId,
  type MemberHasntWorkout,
  useMembersHasntWorkout,
} from "@/hooks/useMemberHasntWorkout";
...
const getMemberName = (member: MemberHasntWorkout) =>
...
const getJoinedInformation = (joined?: string | null) => {
  if (!joined) {
    return { date: "Belum tersedia", duration: "Tanggal bergabung belum ada" };
  }
  const joinedDate = dayjs(joined);
  if (!joinedDate.isValid()) {
    return {
      date: "Belum tersedia",
      duration: "Tanggal bergabung tidak valid",
    };
  }

  const elapsedDays = Math.max(
    dayjs().startOf("day").diff(joinedDate, "day"),
    0,
  );
  return {
    date: joinedDate.format("DD MMM YYYY"),
    duration:
      elapsedDays === 0
        ? "Bergabung hari ini"
        : `${elapsedDays} hari sejak bergabung`,
  };
};

function MemberAvatar({ member }: { member: MemberHasntWorkout }) {
...
function MemberCard({ member }: { member: MemberHasntWorkout }) {
  const name = getMemberName(member);
      ...
      <Pressable
        ...
        onPress={() =>
          void openWhatsApp({
            ...
            message: `Halo ${name}, kami dari Curves ingin mengajak Anda memulai workout bersama. Kami siap membantu mendampingi latihan pertama Anda.`,
          })
        }
...
export default function MemberHasntWorkoutScreen() {
  const { user } = useAuth();
  const clubId = getHasntWorkoutClubId(user?.user_personal?.member_club_id);
  ...
  const query = useMembersHasntWorkout({
    clubId,
  ...
  return (
    <ContainerPage
      titleHeader="Belum Pernah Workout"
      titleContent="Bantu member memulai workout pertamanya"
    >
      <FlatList
        data={members}
              ...
              <View className="flex-row items-center justify-between">
                <View className="mr-4 flex-1">
                  <Text className="text-sm font-medium text-violet-200">
                    Member aktif belum pernah workout
                  </Text>
          ...
          <View className="items-center px-8 py-10">
            ...
            >
              {query.isLoading ? (
                ...
                <Ionicons
                  name={
                    query.isError ? "alert-circle-outline" : "fitness-outline"
                  }
                  size={42}
            ...
            <Text className="mt-2 text-center text-sm leading-5 text-gray-500 dark:text-gray-400">
              {!clubId
                ...
                : query.isError
                  ...
                  : debouncedSearch
                    ? `Tidak ada member bernama “${debouncedSearch}” dalam daftar ini.`
                    : "Tidak ada member aktif yang belum pernah melakukan workout."}
            </Text>
